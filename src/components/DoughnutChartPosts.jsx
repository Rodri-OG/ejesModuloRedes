...
export default function DoughnutChartPosts({ idMetricas }) {
  ...
  useEffect(() => {
    if (metricsData.length > 0) {
      ...
      const totalPosts = metricsData.reduce((acc, metric) => acc + metric.totales.posts, 0);
      const colors = [
        '#F47E36',
        '#497B8E',   
        '#EDD789',   
       ...
       const data = metricsData.map(metric => {
        const percentage = (metric.totales.posts / totalPosts) * 100;
        return percentage;
      });
