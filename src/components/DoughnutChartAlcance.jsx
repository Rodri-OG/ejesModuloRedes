...
export default function DoughnutChartAlcance({ idMetricas }) {
  ...
  useEffect(() => {
    if (metricsData.length > 0) {
      ...
      const totalAlcance = metricsData.reduce((acc, metric) => acc + metric.totales.alcance, 0);
      const colors = [
        '#F47E36',
        '#497B8E',   
        '#EDD789',   
      ...
      const data = metricsData.map(metric => {
        const percentage = (metric.totales.alcance / totalAlcance) * 100;
        return percentage;
      });
