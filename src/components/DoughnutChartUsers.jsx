...
export default function DoughnutChartUsers({ idMetricas }) {
  ...
  useEffect(() => {
    if (metricsData.length > 0) {
      ...
      const totalUsers = metricsData.reduce((acc, metric) => acc + metric.totales.users, 0);
      const colors = [
        '#F47E36',
        '#497B8E',   
        '#EDD789',   
      ...
      const data = metricsData.map(metric => {
        const percentage = (metric.totales.users / totalUsers) * 100;
        return percentage;
      });
